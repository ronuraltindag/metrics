...
mo2 <- lm(data=CASchools, formula=read~str)
summary(mo2)

mo3 <- lm(data=CASchools, formula=read~str+english)
summary(mo3)

mo4 <- lm(data=CASchools, formula=english~str)
summary(mo4)
...
CASchools$noise1 <- sample(1000, size = nrow(CASchools), replace = TRUE)
CASchools$noise2 <- sample(1000, size = nrow(CASchools), replace = TRUE)
CASchools$noise3 <- sample(1000, size = nrow(CASchools), replace = TRUE)
CASchools$str <- CASchools$students/CASchools$teachers

CASchools1 <- CASchools %>%
  dplyr::select(read,str, english, lunch, expenditure)



mo1 <- lm(data=CASchools, formula=read~str+english)
summary(mo1)

mo2 <- lm(data=CASchools, formula=read~str+english+noise1+noise2+noise3)
summary(mo2)

data("CPS1988") 

CPS1988$educ2 <- 2*CPS1988$education 
CPS1988$white <- 0 
CPS1988$white[CPS1988$ethnicity=='cauc'] <- 1
CPS1988$non.white <- 0 
CPS1988$non.white[CPS1988$white==0] <- 1 
  
mo3 <-  lm(data=CPS1988, formula=wage~education)
mo4 <-  lm(data=CPS1988, formula=wage~education + educ2)

summary(mo3)
summary(mo4)

mo5 <-  lm(data=CPS1988, formula=wage~education+white)
summary(mo5)

mo6 <-  lm(data=CPS1988, formula=wage~ education + non.white + white)
summary(mo6)

mo7 <- lm(data=CASchools, formula=read~str+english+lunch+expenditure)
...
mo8 <- lm(data=CASchools, formula=read~str+english+lunch+expenditure)
mo9 <- lm(data=CASchools, formula=read~english+lunch)
...
g1 <- ggplot(data=CASchools, aes(x=income, y=math)) + 
  geom_point() + 
  theme_minimal() + 
  geom_smooth(method='lm', formula= y~x, se=FALSE, colour='blue') 

g2 <- ggplot(data=CASchools, aes(x=income, y=math)) + 
  geom_point() + 
  theme_minimal() + 
  geom_smooth(method='lm', formula=  y ~ x + I(x^2), se=FALSE, colour='red') 

g3 <- ggplot(data=CASchools, aes(x=income, y=math)) + 
  geom_point() + 
  theme_minimal() + 
  geom_smooth(method='lm', formula=  y ~ x + I(x^2) + I(x^3), se=FALSE, colour='#4a1486') 

plot(g1)
plot(g2)
plot(g3)

CASchools$income.sq <- CASchools$income^2
CASchools$income.cu <- CASchools$income^3

mo1 <- lm(data=CASchools, formula=math ~ income)
mo2 <- lm(data=CASchools, formula=math ~ income + income.sq)
mo3 <- lm(data=CASchools, formula=math ~ income + income.sq + income.cu)

stargazer::stargazer(mo1,mo2,mo3, type='text')
mean(CASchools$math)
mean(CASchools$income)

linearHypothesis(mo3, c("income.sq=0", "income.cu=0"))
linearHypothesis(mo3, c("income.cu=0"))
...
```{r eval=T, echo=T, message=FALSE, warning=FALSE, paged.print=TRUE}
hmda <- read_dta("http://fmwww.bc.edu/ec-p/data/stockwatson/hmda_sw.dta")
...
hmda <- hmda %>% 
  dplyr::select(PI, hse_inc, loan_val, ccred, mcred, pubrec, denpmi, selfemp,
                    single, hischl, probunmp, condo, black, deny)

mo1 <- lm(data=hmda, formula=deny ~ PI)
mo2 <- lm(data=hmda, formula=deny ~ black)
mo3 <- lm(data=hmda, formula=deny ~ black + PI)

set.seed(1234)

sub.g1 <- hmda %>%
  sample_n(1000) %>%
  ggplot(aes(x=PI,y=deny)) + 
  geom_point(colour='black', size=0.3, alpha=0.4) + 
...
sub.g2 <- hmda %>%
  filter(PI>0.20) %>%
  sample_n(100) %>%
  ggplot(aes(x=black,y=deny)) + 
  geom_point(colour='blue') + 
  geom_smooth(method=lm, se = FALSE, colour='red')


stargazer(mo1,mo2,mo3, type='text')



mo1 <- lm(data=hmda, formula=deny ~ PI)
mo2 <- lm(data=hmda, formula=deny ~ PI + black)
mo3 <- glm(data=hmda, formula=deny ~ PI ,family=binomial(link='probit')) 
mo4 <- glm(data=hmda, formula=deny ~ PI + black,family=binomial(link='probit')) 


stargazer(mo1,mo2,mo3,mo4, type='text')

values <- data.frame(PI=c(0.25,0.25,0.35,0.35), black=c(0,1))

values$y.hat.lpm1 <-    predict(mo1, newdata = values[1]) 
values$y.hat.lpm2 <-    predict(mo2, newdata = values) 

values$y.hat.probit.mo3 <- predict(mo3, newdata = values[1], type="response") 
values$y.hat.probit.mo4 <- predict(mo4, newdata = values, type="response") 

```

# Post-covid syllabus update 
