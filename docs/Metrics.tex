...
\item
  Empirical project progress report due: \textbf{Nov 1, 2020, midnight}
\item
...
\textbf{Deadline: Sep 27, 2020, Midnight}
...
\textbf{Deadline: Nov 8, 2020, Midnight}
...
\chapter{Replication Project}\label{replication-project}}

\hypertarget{instructions}{%
\section{Instructions}\label{instructions}}

\textbf{a.} Carefully read the study and provide a two paragraph summary that includes:
...
\textbf{b.} Run the following regression for higher and lower quality CVs separately as well as the full sample of CVs. Prepare a regression table of the results and interpret these results within the context of the study.

\(call_i = \beta_0 + \beta_1 black_i + u_i\)

where \(black\) is a dummy variable that equals one for distinctly African-American names.

\textbf{c.} Run separate linear regressions using \texttt{call} as an outcome and \texttt{yearsexp}, \texttt{volunteer}, \texttt{military}, \texttt{email}, \texttt{empholes}, \texttt{workingschool}, \texttt{honors}, \texttt{computerskills}, and \texttt{specialskills}for CVs with African-American and White names. Prepare a regression table and interpret the results within the context of the study. Write up the results that you find interesting and related to the research question in the study, you don't need to interpret each coefficient.

\textbf{d.} Conduct an additional analysis using the data that you have that offer any interesting additional findings to the reported ones in the published study. This could be a subgroup analysis, a graphical representation of a finding in the study or any empirical exploration that you find interesting and use the tools that we learned in the classroom.

\textbf{e.} Conclude your replication study by 1-2 paragraphs of discussion of the results above. What do these ressults imply? What are the policy implications?

\hypertarget{files-data-codebook-study}{%
\section{Files: Data, codebook, study}\label{files-data-codebook-study}}

Start a new R script, copy/paste the header below and save it to \texttt{Dropbox\textbackslash{}EC282\textbackslash{}Empirical\_Project} or a similar path that you created for this course. Run the R script and make sure that you have the data \texttt{df1} in your environment. Conduct the analysis below the header.

\begin{itemize}
\tightlist
\item
  You can find the data codebook \href{https://www.dropbox.com/s/cj8b2sp9n6g6ixq/des.pdf?dl=0}{here}.
\item
  You can find the original study \href{https://www.dropbox.com/s/c6ybtk7v9g8dfbb/Bertrand\%26Mullainathan.pdf?dl=0}{here}
\end{itemize}
...
\CommentTok{###############################################################################}
\CommentTok{#get the data url }
\NormalTok{df1.url <-}\StringTok{ 'https://www.dropbox.com/s/oekcvjgy4yfjv0e/lakisha_aer.dta?dl=1'}
\CommentTok{#download the data }
\KeywordTok{GET}\NormalTok{(df1.url, }\KeywordTok{write_disk}\NormalTok{(tdf <-}\StringTok{ }\KeywordTok{tempfile}\NormalTok{(}\DataTypeTok{fileext =} \StringTok{".dta"}\NormalTok{)))}
\CommentTok{#check if it worked}
\NormalTok{df1 <-}\StringTok{ }\KeywordTok{read_dta}\NormalTok{(tdf) }
